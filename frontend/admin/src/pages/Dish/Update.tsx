...
const PictureUpload = ({
  ...
  return (
    <div className="flex gap-2">
      {value && (
        <div className="flex-center flex-col w-24 h-24 border-dashed border border-gray-300 rounded-lg cursor-pointer">
          <Image src={value} className="!w-20 !h-20" />
        </div>
      )}
      ...
      >
        <div className="flex-center flex-col w-24 h-24 border-dashed border border-gray-300 rounded-lg cursor-pointer">
          {loadig ? (
            ...
            <>
              <PlusOutlined />
              <div className="mt-2">{value ? "修改" : "上传"}</div>
            </>
          )}
        </div>
      </Upload>
...
const Page = () => {
  ...
  const onFinish = async (values: IForm) => {
    try {
      if (id) {
        await api.dish.updateUsingPut1({ ...values, id: +id });
